...
from .models import CodexRunResult, IncomingMessage, RemoteAgent, RemoteConfig, RunRecord, SessionBinding
...
class CommandHandlers:
    ...
    def __init__(
        ...
        self.run_manager = RunManager(state, codex_runner)
    ...
    async def _new_session(self, msg: IncomingMessage, command: Command) -> None:
        repo_alias, title, prompt = self._parse_new_args(command.args)
        if not prompt and title:
            prompt = _standby_prompt(title)
        if not prompt:
            await self.lark.reply(msg.message_id, "用法：/new <repo> <title> [任务]，例如 `/new agent agent-console 检查当前状态`。")
            return
        ...
        await self.lark.reply(msg.message_id, f"收到，开始创建线上专员 `{title or _title_from_prompt(prompt)}`。")
        ...
        if result.session_id:
            ...
            self.run_manager.attach_run_to_agent(run.id, agent.id, result.session_id)
    ...
    async def _continue_session(self, msg: IncomingMessage, prompt: str) -> None:
        ...
        try:
            run, result = await self.run_manager.resume_agent(
                ...
                codex_session_id=binding.codex_session_id,
                prompt=prompt,
            )
    ...
    async def _agents(self, msg: IncomingMessage, command: Command) -> None:
        ...
        for agent in agents:
            ...
            lines.append(
                ...
                f"session={agent.codex_session_id} status={status} last_run={agent.last_run_id or '-'} updated={agent.updated_at}"
            )
    ...
    async def _codex_sessions(self, msg: IncomingMessage, command: Command) -> None:
        limit = _parse_limit(command.args, default=10)
        sessions = self.session_finder.recent(limit=limit)
        if not sessions:
            await self.lark.reply(msg.message_id, "没有找到本机 Codex session。")
            return
        lines = [f"- {item.session_id} cwd={item.cwd} time={item.timestamp or '-'} source={item.source or '-'}" for item in sessions]
        suffix = "\n提示：旧命令 `/recent-codex` 仍可用，但建议改用 `/codex-sessions`。" if command.raw_name == "recent-codex" else ""
    ...
    async def _attach(self, msg: IncomingMessage, command: Command) -> None:
        ...
        if args and not args.startswith("repo="):
            ...
            if agent is not None:
                switch_text = self._bind_agent(msg, agent.id, agent.repo_alias, agent.repo_path, agent.codex_session_id)
                await self.lark.reply(msg.message_id, switch_text)
                return
    ...
    async def _handoff(self, msg: IncomingMessage, command: Command) -> None:
        agent, repo_alias, repo_path, session_id = self._resolve_handoff_target(msg, command.args)
        if session_id is None or repo_path is None or repo_alias is None:
            ...
            return
        prompt = (
            "请生成这个线上专员/Codex session 的标准交接总结。请用中文，包含：目标、已完成工作、"
    ...
    async def _switch_repo(self, msg: IncomingMessage, command: Command) -> None:
        ...
        self.state.upsert_session(msg.chat_id, _binding_key(msg), alias, repo_path, binding.codex_session_id, agent_id=binding.agent_id)
    ...
    async def _doctor(self, msg: IncomingMessage, command: Command) -> None:
        ...
        checks = [
            ...
            _check_line("codex executable", bool(codex_path), self.config.codex_bin),
            _check_line("codex version", await _command_ok([self.config.codex_bin, "--version"]) if codex_path else False, "--version"),
    ...
    def _update_binding_after_result(self, msg: IncomingMessage, binding: SessionBinding, result: CodexRunResult) -> None:
        session_id = result.session_id or binding.codex_session_id
        self.state.upsert_session(msg.chat_id, _binding_key(msg), binding.repo_alias, binding.repo_path, session_id, agent_id=binding.agent_id)
        if binding.agent_id and result.session_id:
            self.state.update_remote_agent_session(binding.agent_id, result.session_id)

    def _bind_agent(self, msg: IncomingMessage, agent_id: str, repo_alias: str, repo_path: Path, codex_session_id: str) -> str:
        key = _binding_key(msg)
        previous = self.state.get_session(msg.chat_id, key)
        self.state.upsert_session(msg.chat_id, key, repo_alias, repo_path, codex_session_id, agent_id=agent_id)
    ...
    def _resolve_handoff_target(self, msg: IncomingMessage, args: str) -> tuple[RemoteAgent | None, str | None, Path | None, str | None]:
        stripped = args.strip()
        if stripped.startswith("repo="):
            repo_alias, session_id = self._parse_repo_arg(stripped)
            repo_path = self._repo_path(repo_alias)
            return None, repo_alias, repo_path, session_id
        if stripped:
            agent = self.state.get_remote_agent(stripped)
            if agent:
                return agent, agent.repo_alias, agent.repo_path, agent.codex_session_id
        binding = self._current_binding(msg)
        if binding:
            agent = self.state.get_remote_agent(binding.agent_id) if binding.agent_id else None
            return agent, binding.repo_alias, binding.repo_path, binding.codex_session_id
