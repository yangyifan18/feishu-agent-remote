...
class CodexRunner:
    ...
    async def start(self, repo_path: Path, prompt: str) -> CodexRunResult:
    ...
    async def resume(self, session_id: str, repo_path: Path, prompt: str) -> CodexRunResult:
        return await self._run(
    ...
    async def _run(self, argv: list[str], cwd: Path | None = None) -> CodexRunResult:
        ...
        try:
            ...
            proc.kill()
            await proc.wait()
            return CodexRunResult(session_id=None, summary="Codex 执行超时，已终止。")
        ...
        if proc.returncode != 0:
            ...
            return CodexRunResult(session_id=parsed.session_id, summary=f"Codex 执行失败：{summary}")

        return CodexRunResult(session_id=parsed.session_id, summary=parsed.last_message or "Codex 已完成，但没有返回文本。")
