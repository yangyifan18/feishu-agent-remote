...
class EventReliabilityTests(unittest.TestCase):
    ...
    def test_event_consumer_backs_off_when_process_factory_fails(self):
        async def run():
            ...
            async def process_factory(*args, **kwargs):
                raise FileNotFoundError("missing lark-cli")
    ...
    def test_event_consumer_resets_delay_after_stable_run(self):
        async def run():
            sleeps = []
            processes = [FakeProcess(), FakeProcess(), FakeProcess()]
            clock_values = iter([0.0, 0.0, 0.0, 61.0, 61.0, 61.0])
    ...
    def test_incoming_event_extracts_thread_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            config_path = Path(tmp) / "config.yaml"
            state_path = Path(tmp) / "state.sqlite"
            config_path.write_text(
            ...
            try:
                ...
                self.assertEqual(msg.thread_id, "omt_thread")
                self.assertEqual(msg.root_message_id, "om_root")
            finally:
                for key, value in old_env.items():
                    if value is None:
                        os.environ.pop(key, None)
